...
class PhysicsDumper():
    ...
    def dumpData(self, obj):
        ...
        for b in obj.bd:
            ...
            # body: Optional[pymunk.Body] = obj.bd[b]
            # xmlbody: Optional[ET.SubElement] = ET.SubElement(bodies, 'body')
            # if body.body_type == pymunk.Body.DYNAMIC:
            #     xmlbody.set("type", "DYNAMIC")
            # elif body.body_type == pymunk.Body.KINEMATIC:
            #     xmlbody.set("type", "KINEMATIC")
            ...
            # xmlbody.set("moment", str(body.moment))
            #
            # self.dumpShape(xmlbody, body.shapes, obj)
...
'b' to reload bike, 'n' to reload level'''
    ...
    def setup(self):
        ...
        self.level = Level()
        self.level2 = Level2()
        self.level2.getTexture(EXEC_FOLDER + '\\mapa.png').getMarchingCubes(60, 60).sendSegmentsToPhysics(self.physics_engine.space)
    ...
    def on_draw(self):
        ...
        if self.drawGraphics:
            ...
            #arcade.draw_scaled_texture_rectangle(self.level2.t.width, self.level2.t.height, self.level2.t, self.level2.scale_x)
